...
proc toDom*(n: VNode; useAttachedNode: bool; kxi: KaraxInstance = nil): Node =
  ...
  if n.kind == VNodeKind.text:
    ...
    attach n
    return result
  else: # we are dealing with fundamental tags at this point
    if svgElements.contains(n.kind):
      result = document.createElementNS(svgNamespace,toTag[n.kind]) 
      #svg elements need to be declared with a namespace
